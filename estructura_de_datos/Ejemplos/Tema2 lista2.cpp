...
{
    Tlista q;   // nodo auxiliar
    q = new(struct nodo);  //asignar nuevo espacio de memoria
    q->nro = valor;  //asignamos valor al campo de información
    q->sgte = lista; // direccionamiento
    lista  = q;  // direccionamiento
}
...
void insertarElemento(Tlista &lista, int valor, int pos)
{
    Tlista q, t;
    int i;
    q = new(struct nodo);  // crear un nuevo espacio en memoria para la estructura nodo
    q->nro = valor;
...
void buscarElemento(Tlista lista, int valor)
{
    ...
    while(q!=NULL)  // ciclo para recorrer nuestra lista hasta el final
    {
        if(q->nro==valor)
        {
        ...
        q = q->sgte; // estar avanzando de nodo
        i++;
    }
...
void reportarLista(Tlista lista)
{
     ...
     while(lista != NULL) // recorrido de la lista
     {
